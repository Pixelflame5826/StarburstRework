...
function init()
  ...
  self.regenModifier = config.getParameter("regenModifier", 1.0)
  self.bashDamage = config.getParameter("bashDamage", 0) * root.evalFunction("weaponDamageLevelMultiplier", self.level)
  self.bashDamageType = config.getParameter("bashDamageType", "default")
...
function raiseShield()
  ...
  self.active = true
  self.activeTimer = 0
  status.setPersistentEffects(activeItem.hand().."Shield", {{stat = "shieldHealth", amount = shieldHealth()}, {stat = "shieldStaminaRegen", effectiveMultiplier = self.regenModifier }})
  local shieldPoly = animator.partPoly("shield", "shieldPoly")
  activeItem.setItemShieldPolys({shieldPoly})

  if (self.knockback > 0) or (self.bashDamage > 0) then
    local knockbackDamageSource = {
      poly = shieldPoly,
      damage = self.bashDamage,
      damageType = "Knockback",
      sourceEntity = activeItem.ownerEntityId(),
      team = activeItem.ownerTeam(),
      knockback = self.knockback,
      damageSourceKind = self.bashDamageType,
