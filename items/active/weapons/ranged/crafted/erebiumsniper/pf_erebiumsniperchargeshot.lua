...
function pf_ChargeShot:charge()
  ...
  while self.fireMode == (self.activatingFireMode or self.abilitySlot) do
	...
	if (self.soundCue and (self.chargeTimer >= 0.2)) then
	  animator.playSound("charging")
	  self.soundCue = false
	end

	if self.chargeTimer >= 1.2 then
	  activeItem.setCursor("/cursors/pf_erebiumsnipercharged.cursor")
	else
	  activeItem.setCursor("/cursors/pf_erebiumsnipercharging.cursor")
	end
