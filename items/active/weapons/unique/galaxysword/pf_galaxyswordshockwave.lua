...
function ShockWave:fire(charge)
  ...
  local timer = self.stances.fire.duration
  while timer > 0 do
    if timer <= (self.stances.fire.duration - 0.2) then
	  if self.fireMode == "primary" and self.extraAttack then
        self:fireShockwave(charge)
