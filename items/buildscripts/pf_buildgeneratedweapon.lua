...
function build(directory, config, parameters, level, seed)
  ...
  local statLevel = configParameter("level", 1)
  if (statLevel > 1) then
    statLevel = statLevel - 0.5
  end
  config.damageLevelMultiplier = root.evalFunction("weaponDamageLevelMultiplier", statLevel)
