...
function populateList(collectionName)
  ...
  if self.collectionName then
    ...
    widget.setText("topicLabel", root.assetJson(string.format("/interface/scripted/starburstcollections/pf_starburstcollections_data.config:%s.topic", self.collectionName)))
    widget.setText("descriptionLabel", root.assetJson(string.format("/interface/scripted/starburstcollections/pf_starburstcollections_data.config:%s.description", self.collectionName)))
...
function createTooltip(screenPosition)
  for widgetName, collectable in pairs(self.currentCollectables) do
    if widget.inMember(widgetName, screenPosition) and self.playerCollectables[collectable.name] then
      ...
      tooltip.icon.file = collectable.icon
      return tooltip
    end
  end
end
